...
## 24. シェル機能拡張・プロンプト・ターミナル効率化

### 🎨 [Oh My Posh](https://github.com/JanDeDobbeleer/oh-my-posh)
* **カテゴリ**: クロスシェル・プロンプトテーマエンジン
* **概要**: Go で書かれた、非常にカスタマイズ性が高く高速なプロンプトテーマエンジン。Zsh, Fish, Bash, PowerShell, NuShell などほぼ全てのシェルをサポート。豊富なビルトインテーマやセグメント（Git, AWS, Node, K8s, バッテリー状態等）を提供。
* **採用検討理由**: Starship よりさらに複雑でグラフィカルなセグメント（過剰なまでのアイコン装飾や色グラデーション）や、詳細なカスタマイズを行いたい場合の強力な選択肢です。
* **主な競合・代替ツール**: Starship, Powerlevel10k, Spaceship Prompt

### 🔍 [fzf-tab](https://github.com/Aloxaf/fzf-tab)
* **カテゴリ**: Zsh 補完 UI 拡張
* **概要**: Zsh のデフォルトのタブ補完選択メニューを、インタラクティブな `fzf` のファジー検索 UI に置き換えるプラグイン。
* **採用検討理由**: 通常の Zsh の矢印キーやタブ連打による補完選択を、高速なあいまい検索とプレビュー（`bat` や `eza` などと連携してファイル内容やディレクトリ構造をプレビュー可能）にアップグレードし、コマンド入力効率を極限まで引き上げます。
* **主な競合・代替ツール**: zsh-autocomplete, fzf-autocomplete

### ⚡ [Carapace](https://github.com/rsteube/carapace-bin)
* **カテゴリ**: マルチシェル・コマンド引数補完ジェネレーター
* **概要**: Go で書かれた超高速かつ強力なコマンド補完エンジン。`docker`, `git`, `aws`, `kubectl` などの数百もの主要な CLI コマンドに対し、Zsh, Bash, Fish, Nushell, PowerShell で共通の高度な補完機能を提供。
* **採用検討理由**: 各シェル固有の補完定義ファイルが遅かったり不十分だったりする問題を解決し、どのシェルでも均一で爆速なサブコマンド・オプション補完を実現します。
* **主な競合・代替ツール**: 各シェル標準の補完, completion-generator

### 🧠 [McFly](https://github.com/cantino/mcfly)
* **カテゴリ**: AIベース（ニューラルネットワーク）シェル履歴検索
* **概要**: Rust で開発された、シェルの実行履歴をファジー検索するツール。ニューラルネットワークを利用して「現在のディレクトリ」「直前のコマンドの成功/失敗」「使用頻度」などを加味して、最も実行したい可能性が高いコマンドを提案します。
* **採用検討理由**: `Atuin` や通常の `Ctrl+R` (`fzf`) 履歴検索の代替として、コンテキストに応じたインテリジェントでスマートなコマンド再実行体験をもたらします。
* **主な競合・代替ツール**: Atuin, fzf (Ctrl+R 検索)

### 🐙 [forgit](https://github.com/wfxr/forgit)
* **カテゴリ**: fzf 統合対話型 Git アシスタント
* **概要**: `fzf` を利用して、`git add`, `git log`, `git diff`, `git checkout` などの操作をインタラクティブかつグラフィカルに行えるシェルプラグイン/エイリアス集。
* **採用検討理由**: コマンドの引数やハッシュ値を手入力することなく、コミット履歴のプレビュー、差分を見ながらの部分ステージング、ブランチの切り替えなどを高速に行えます。
* **主な競合・代替ツール**: gitui, lazygit, tig

### 🌊 [Tide](https://github.com/IlanCosman/tide)
* **カテゴリ**: Fish シェル向け超高速非同期プロンプト
* **概要**: Fish シェルに特化した、モダンで非常に美しい非同期プロンプトフレームワーク。コマンドの実行をブロックせずに、Git の状態やシステム情報を裏側で取得してプロンプトを描画します。
* **採用検討理由**: 将来的に Fish シェルをメインに採用する場合、Rust 製の Starship に匹敵する表示速度と、Fish ネイティブな拡張性の高さを両立させるプロンプトとして最適です。
* **主な競合・代替ツール**: Starship, Oh My Posh, Hydro

### 🚀 [fastfetch](https://github.com/fastfetch-cli/fastfetch)
* **カテゴリ**: システム情報表示 (Neofetch 代替)
* **概要**: ディストリビューションのロゴや OS・ハードウェアのスペック、シェルやターミナルの情報をアスキーアートと共に表示する `neofetch` の超高速 C言語製後継ツール。
* **採用検討理由**: 開発終了した `neofetch` に代わり、最新のハードウェア（Apple Silicon, Windows Terminal, 各種 Linux）に対応し、ターミナル起動時のスタートアップ処理に組み込んでも一切ラグが発生しない速度でシステムステータスを出力できます。
* **主な競合・代替ツール**: neofetch (開発終了), archey, screenfetch
...
* **採用検討理由**: `rm -rf` による誤消去を防止しつつ、削除履歴の確認や、元に戻す (`rip -u`) 操作も容易で、ターミナル作業の安全性を高めます。
