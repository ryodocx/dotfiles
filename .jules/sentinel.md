...
## 2024-08-25 - Secret exposure via wrapper script CLI arguments

**Vulnerability:** A parent wrapper script (`install.sh`) accepted the GitHub token as a command-line argument (`--github-token`). Like passing secrets to subprocesses, passing secrets to wrapper scripts exposes them in the system's process list (`ps`) and shell history.
**Learning:** Accepting secrets via CLI arguments in wrapper scripts poses the same security risks as passing them to subprocesses.
**Prevention:** Parent wrapper scripts should read secrets securely via environment variables or interactive prompts, and avoid accepting them as command-line arguments.
