...
## 2025-02-27 - Do not pass secrets via CLI arguments

**Vulnerability:** The setup script `install.sh` accepted a GitHub Personal Access Token via the `--github-token` command-line argument.
**Learning:** Passing sensitive information (e.g., tokens, secrets, passwords) via command-line arguments is highly insecure because the arguments are visible to all users on the system via process lists (e.g., using `ps aux`).
**Prevention:** Always use environment variables, secure configuration files, or prompt for sensitive data securely (like using `read -s`). `install.sh` was updated to read from `DOTFILES_GITHUB_TOKEN` instead of `--github-token`.
